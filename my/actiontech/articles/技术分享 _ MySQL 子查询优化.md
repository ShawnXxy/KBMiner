...
- 
- 
`SELECT t2.b FROM t2 WHERE id < 10; ``结果:1,2,3,4,5,6,7,8,9 ``select * from t1 where t1.a in(1,2,3,4,5,6,7,8,9);`
...
- 
- 
`select ``    `t1`.`id`,`t1`.`a`,`t1`.`b` ``from `t1` semi join `t2` ``where``    ((`t1`.`a` = `<subquery2>`.`b`) ``    and (`t2`.`id` < 10)); ``##注意这是优化器改写的SQL，客户端上是不能用 semi join 语法的 ``+----+--------------+-------------+-------+---------+---------------+------+-------------+``| id | select_type  | table       | type  | key     | ref           | rows | Extra       |``+----+--------------+-------------+-------+---------+---------------+------+-------------+``|  1 | SIMPLE       | <subquery2> | ALL   | NULL    | NULL          | NULL | Using where |``|  1 | SIMPLE       | t1          | ref   | a       | <subquery2>.b |    1 | NULL        |``|  2 | MATERIALIZED | t2          | range | PRIMARY | NULL          |    9 | Using where |``+----+--------------+-------------+-------+---------+---------------+------+-------------+`
